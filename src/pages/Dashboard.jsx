  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <Link 
          to="/profile-settings" 
          className="w-full sm:w-auto px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 text-center"
        >
          Profile Settings
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              ...
              <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">{stats?.total_projects || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-primary-100 p-2 sm:p-3 rounded-full flex-shrink-0 ml-2">
              <svg className="w-6 sm:w-8 h-6 sm:h-8 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {stats?.total_tasks && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            ...
            <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">{stats.total_tasks}</p>
          </div>
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <p className="text-xs sm:text-sm text-gray-600">Total Transactions</p>
            <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">{stats.total_transactions || 0}</p>
          </div>
        </div>
      )}
                ...
                {projects.map((project) => (
                  <tr key={project.id} className="hover:bg-gray-50">
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="text-xs sm:text-sm font-medium text-gray-900 truncate">{project.project_name}</div>
                    </td>
                    <td className="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-green-600">
                      RWF {project.total_income?.toLocaleString('en-US') || '0'}
                    </td>
                    <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-red-600">
